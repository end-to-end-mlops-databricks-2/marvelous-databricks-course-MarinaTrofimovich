...
def create_or_refresh_monitoring(config, spark, workspace):
    inf_table = spark.sql(
        f"SELECT * FROM {config.catalog_name}.{config.schema_name}.`churn_predictor-model-serving_payload_payload`"
    )
    ...
    df_final_with_status = (
        df_final.join(test_set.select("CustomerId", "Exited"), on="CustomerId", how="left")
        ...
        .select("*", F.coalesce(F.col("Exited_test"), F.col("Exited_inference")).alias("Exited"))
