...
query = w.queries.create(query=sql.CreateQueryRequestQuery(display_name=f'classification-alert-query-{time.time_ns()}',
...
alert = w.alerts.create(
    alert=sql.CreateAlertRequestAlert(condition=sql.AlertCondition(operand=sql.AlertConditionOperand(
        column=sql.AlertOperandColumn(name="percentage_lower_than_85")),
